...
class TransferLearningModel(pl.LightningModule):
    ...
    def on_epoch_start(self):
        ...
        if self.current_epoch == self.milestones[0]:
            _unfreeze_and_add_param_group(
                module=self.feature_extractor[-2:],
                optimizer=optimizer,
    ...
    def training_epoch_end(self, outputs):
        ...
        train_acc_mean = (
            torch.stack([output["num_correct"] for output in outputs]).sum().float()
        )
    ...
    def prepare_data(self):
        ...
        download_and_extract_archive(
            url=DATA_URL, download_root=self.dl_path, remove_finished=True
        )
